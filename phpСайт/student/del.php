...
if (!isset($id_student)) {
    $_SESSION['error_message'] = "ID Ученика не указан";
    header("Location: list.php");
    exit();
}
...
$query = "DELETE FROM public.students WHERE id_student = :id_student";
$stmt = $pdo->prepare($query);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $stmt->execute(['id_student' => $id_student]);
        $_SESSION['success_message'] = "Ученик успешно удалён";
        header("Location: list.php");
        exit();
    } catch (PDOException $e) {
...
<div class="container-fluid p-2" id="upbar" style="position: fixed; background-color: rgba(141, 141, 141, 1); z-index: 2;">       
        <div class="col">
            <div class="row align-items-start">
                ...
                <div class="col px-2 text-center"><h4>Удалить Ученика</h4></div>
            </div>
        </div>
    </div>
